...
from architectures import ODENet, DerivativeMLP, DerivativeMLP2, StateTrajectoryNetwork, ExcitationSecondsLinearInterpolation, ResidualIntegrationNetworkRK4, BilinearBlock, ScaledODENetFE, DerivativeMLPFE, DerivativeMLP2FE
...
def main():
    ...
    networks = [SimpleRNN(unit_type="LSTM", hidden_size=8, skip=0),
                ...
                SimpleRNN(unit_type="LSTM", hidden_size=32, skip=0),
                ODENet(DerivativeMLP(ExcitationSecondsLinearInterpolation(), nn.Softsign(), excitation_size=1, output_size=2, hidden_size=30), ForwardEuler(), dt, 2),
