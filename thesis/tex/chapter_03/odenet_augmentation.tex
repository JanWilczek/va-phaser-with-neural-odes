...
\begin{enumerate}
  ...
  \item How to implement the ODENet in the context of audio signal processing? (\Section{subsec:implementation})
...
\begin{equation}
  ...
  \end{bmatrix},
\end{equation}
where $y[0]$ is the audio output compared to the target data, and $y[1], \dots, y[M]$ are unknown states with $M$ fixed by a specialist that the network should populate on its own.
