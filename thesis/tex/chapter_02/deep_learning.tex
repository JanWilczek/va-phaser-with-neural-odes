...
\begin{figure}
  \centering
  \scalebox{0.9}{\input{figures/svg/stn.pdf_tex}}
  \caption{\Acf{STN} architecture. After \cite{Parker2019}.}
  \label{fig:stn}
\end{figure}

The architecture of \ac{STN} is shown in \Figure{fig:stn}. The layers $l_1, \dots, l_L$ are an \ac{MLP}. A novelty in \ac{STN} in comparison to \ac{ResNet} is the presence of the residual scaling factor $h_\tau$. In \cite{Parker2019}, it is defined as
...
One of such methods is the \acf{RINN} \cite{Ouala2019}.
...
\begin{figure}
    \centering
    \scalebox{0.8}{\input{figures/svg/teacher_forcing.pdf_tex}}
