...
\end{equation}
where $\pmb{W}^{(l)}$ is the weight matrix of the $l$-th layer with the $i$-th row associated with the $i$-th unit in the layer, $\pmb{b}$ is a vector of units' biases, $g(\cdot)$ is a nonlinear function applied element wise, and $\pmb{y}^{(0)}$ is the network's input vector. In this notation, $f(\pmb{y}^{(0)};\pmb{\theta}) = \pmb{y}^{(L)}$.
...
A memory cell $i$ has an associated state at time step $t$ denoted $c_t$. The output of a memory cell is the \emph{hidden state} $h_t$, which is an element-wise (Hadamard) product of the output gate and the cell state processed by $\tanh$

\begin{equation}
  h_t = o_t \odot \tanh (c_t).
\end{equation}

Output gate's purpose is to determine whether the cell can produce output at time step $t$. Output gate is a function of the current input and previous hidden state

\begin{equation}
  o_t = \sigma (\pmb{W}_{io} \pmb{x}_t + \pmb{b}_{io} + \pmb{W}_{ho} \pmb{h}_t + \pmb{b}_{ho}),
\end{equation}

where $\pmb{W}_{io}$ and $\pmb{W}_{ho}$ are output gate's weight matrices, $\pmb{b}_{io}$ and $\pmb{b}_{ho}$ are the output gate's bias vectors, and $\sigma$ denotes the sigmoid function applied element-wise.

% TODO: Was sigmoid defined at this point?

Input gate vector $\pmb{i}_t$ and forget gate vector $\pmb{f}_t$ are defined analogously

\begin{align}
  \pmb{i}_t &= \sigma (\pmb{W}_{ii} \pmb{x}_t + \pmb{b}_{ii} + \pmb{W}_{hi} \pmb{h}_{t-1} + \pmb{b}_{hi}), \\
  \pmb{f}_t &= \sigma (\pmb{W}_{if} \pmb{x}_t + \pmb{b}_{if} + \pmb{W}_{hf} \pmb{h}_{t-1} + \pmb{b}_{hf}).
\end{align}

The input and forget gates control the amount of information introduced to the cell state from the current input and the amount of information preserved from the previous cell state respectively. Finally, the cell state is updated as follows
...
\ac{LSTM} has been successfully applied to guitar amplifier modeling \cite{Wright2019,Wrightetal2020} and time-varying effects modeling \cite{Wright2020}.
...
Backpropagation is an algorithm of calculating the gradient of the loss function with respect to the model parameters. 
