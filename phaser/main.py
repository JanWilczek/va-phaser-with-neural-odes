...
def get_architecture(args, dt):
    if args.method == 'LSTM':
        ...
        network = ODENet(DerivativeMLP(ExcitationSecondsLinearInterpolation(), get_nonlinearity(args), excitation_size=2, output_size=args.state_size, hidden_size=args.hidden_size), method, dt)
        # network = ODENet(DerivativeMLP2(ExcitationSecondsLinearInterpolation(), get_nonlinearity(args), excitation_size=2, output_size=args.state_size, hidden_size=args.hidden_size), method, dt)
    return network
