...
def get_architecture(args, dt):
    if args.method == 'LSTM':
        ...
        network = ODENet(DerivativeMLP2(ExcitationSecondsLinearInterpolation(), get_nonlinearity(args), input_size=3, output_size=1, hidden_size=args.hidden_size), method, dt)
    return network
